...
export default function RifiutaCorsaModal(props) {
    ...
    function rifiutaCorsa(e) {
        e.preventDefault()
        setState({ ...state, submit: true })
        try {
            axios.put("/gestione-corse/rifiutaCorsa", { id: props.idPrenotazione, autista: session.id })
                .then(res => {
    ...
    return (
        <Modal
            {...props}
            size="lg"
            aria-labelledby="rifiutaCorsaModal"
            centered>
            <Modal.Header>
                <Modal.Title className="t-bold" id="rifiutaCorsaModal">
                    Rifiuta corsa
                </Modal.Title>
            </Modal.Header>
            <Modal.Body>
                {state.error.show || state.success.show ?
                    ...
                    : <Row className="gy-4" >
                        <Col >
                            <h3 className="t-bold text-center h5">Sei sicuro di voler rifiutare la corsa?</h3>
                        </Col>
                        <div className="buttonsGroup mx-auto">
                            <Button variant={"Secondary"} onClick={props.onHide}>Annulla</Button>
                            <Button spinner={state.submit} variant={"Danger"} onClick={rifiutaCorsa}>Conferma</Button>
                        </div>
                    </Row>
                }
            </Modal.Body>
